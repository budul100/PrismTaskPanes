...
namespace PrismTaskPanes.Factories
{
        ...
        : IDisposable
    {
        ...
        private readonly Dictionary<string, CustomTaskPane> taskPanes = new();
        ...
        {
            var visibleConfigs = configurationsRepository
                .Get(documentHashGetter.Invoke())
                .Where(c => c.Visible
                    && !c.InvisibleAtStart).ToArray();

            foreach (var visibleConfig in visibleConfigs)
            {
                SetTaskPaneVisible(
                    receiverHash: visibleConfig.ReceiverHash,
                    isVisible: true);
            }
        }
        ...
        private void OnTaskPaneDispose(NetOffice.OnDisposeEventArgs eventArgs)
        {
            var taskPane = taskPanes
                .SingleOrDefault(t => t.Value == eventArgs.Sender);
